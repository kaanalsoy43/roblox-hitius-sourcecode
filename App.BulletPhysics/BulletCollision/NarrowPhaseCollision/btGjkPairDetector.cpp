...
bool gUseTriangleNormalForConvexTriangle = false;
...
//		if (check2d)
		...
		/*
		The bellow section is commented out to fix interpenetration and reversed normals. Based on Erwin's patch from Bullet 2.82 to 2.83alpha
		https://github.com/bulletphysics/bullet3/commit/7f3cbc066ff8e72c48e8b292b0b0d6fc415d401c#diff-8010ec2c194975ca94d7b10fb5026bd9
		*/
		...
		if (gUseTriangleNormalForConvexTriangle)
        {
            if (m_shapeTypeA == TRIANGLE_SHAPE_PROXYTYPE)
            {
    			static_cast<const btTriangleShape*>(m_minkowskiA)->calcNormal(normalInB);
                normalInB = -normalInB;
            }
            else if (m_shapeTypeB == TRIANGLE_SHAPE_PROXYTYPE)
            {
    			static_cast<const btTriangleShape*>(m_minkowskiB)->calcNormal(normalInB);
            }
        }
