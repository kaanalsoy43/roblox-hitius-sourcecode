...
bool gFixBulletGJKOptimization = false;
bool gFixBulletGJKOptimization2 = false;
...
int btVoronoiSimplexSolver::pointOutsideOfPlane(const btVector3& p, const btVector3& a, const btVector3& b, const btVector3& c, const btVector3& d)
{
	...
	if (gFixBulletGJKOptimization)
	{
		if (signd * signd < (btScalar(1e-8) * btScalar(1e-8) * normal.length2()))
		{
			return 1;
		}
	}
	else if (gFixBulletGJKOptimization2)
	{
		if (signd * signd < (btScalar(1e-8) * btScalar(1e-8) * normal.length2()))
		{
			return -1;
	...
	if (gFixBulletGJKOptimization)
	{
		if (signd * signd < (btScalar(1e-4) * btScalar(1e-4) * normal.length2()))
		{
			// normal is not normalized, so for a large surface you get a HUGE normal vector
			// this means if point 'd' lies on the same plane as the triangle ABC 
			// signd * signp can be undefined due to floating point error multiplied by a large
			// magnitude.

			// If 'd' is on the same plane as 'abc', we should brute force check this triangle
			// because we cannot be sure whether it is relevant or not! 

			// returning -1 stops GJK Iterations and forces us to use last best found point. Why?
			return 1;
		}
	}
	else if (gFixBulletGJKOptimization2)
	{
		if (signd * signd < (btScalar(1e-4) * btScalar(1e-4) * normal.length2()))
		{
			// In case the above fix causes issues in the future, we can default to this fix.
			// This at least deals with the fact that for a large face, normal is not normalized and
			// for floating point checks to be valid we must multiply the scalars by the normal's length.
			return -1;
		}
	}
	else
