...
static void traceexec (lua_State *L, const InstructionV *pc) {
  lu_byte mask = L->hookmask;
  const InstructionV *oldpc = L->savedpc;
  L->savedpc = pc;
...
void luaV_execute (lua_State *L, int nexeccalls) {
 ...
 reentry:  /* entry point */
  ...
  for (;;) {
    ...
    lua_assert(L->top == L->ci->top || luaG_checkopenop(rbxDecodeOp(iOp, pcRel, L->l_G->ckey)));

    /* warning!! several calls may realloc the stack and invalidate `ra' */
    switch (GET_OPCODE(iOp.p)) {
      case OP_MOVE: {
        const Instruction i = rbxDecodeOp(iOp, pcRel, ckey);
        StkId ra = RA(i);
        setobjs2s(L, ra, RB(i));
        lua_rbx_encoding_check(L,i);
        continue;
      }
      case OP_LOADK: {
        const Instruction i = rbxDecodeOp(iOp, pcRel, ckey);
      ...
      case OP_CONCAT: {
        const Instruction i = rbxDecodeOp(iOp, pcRel, ckey);
        // ra unused (or rather it *is* used but is read below)
        int b = GETARG_B(i);
        int c = GETARG_C(i);
        Protect(luaV_concat(L, c-b+1, c); luaC_checkGC(L));
        setobjs2s(L, RA(i), base+b); /* previous call may change the stack */
        continue;
      }
      case OP_JMP: {
        const Instruction i = rbxDecodeOp(iOp, pcRel, ckey);
      ...
      case OP_EQ: {
        ...
        TValue *rb = RKB(i);
        TValue *rc = RKC(i);
        // rbx assumption: the next opcode is a jump.
        Protect(
          if (equalobj(L, rb, rc) == GETARG_A(i))
            dojump(L, pc, GETARG_sBx(rbxDecodeOpFast(*pc, ckey)));
        )
        pc++;
        continue;
      }
      ...
      case OP_RETURN: {
        ...
        if (--nexeccalls == 0)  /* was previous function running `here'? */
          ...
          if (b) L->top = L->ci->top;
          lua_assert(isLua(L->ci));
          lua_assert(GET_OPCODE(rbxDecodeOpFast(*((L->ci)->savedpc - 1), ckey)) == OP_CALL);
          goto reentry;
        }
      }
      case OP_FORLOOP: {
      ...
      case OP_TFORLOOP: {
        ...
        if (!ttisnil(cb)) {  /* continue loop? */
          ...
          dojump(L, pc, GETARG_sBx(rbxDecodeOpFast(*pc, ckey)));  /* jump back */
        }
        pc++;
        continue;
      }
      case OP_SETLIST: {
        ...
        if (c == 0) c = cast_int(rbxDecodeFakeOp(*pc++, ckey));
      ...
      case OP_CLOSURE: {
        ...
        ncl->l.p = p;
        for (j=0; j<nup; j++, pc++) {
          const Instruction ii = rbxDecodeOpFast(*pc, ckey);
          if (GET_OPCODE(ii) == OP_GETUPVAL)
            ncl->l.upvals[j] = cl->upvals[GETARG_B(ii)];
          else {
            lua_assert(GET_OPCODE(ii) == OP_MOVE);
            ncl->l.upvals[j] = luaF_findupval(L, base + GETARG_B(ii));
          }
        }
        setclvalue(L, ra, ncl);
        Protect(luaC_checkGC(L));
