// RobloxExtensionApp.cpp : Defines the entry point for the console application.
//
//
//  main.cpp
//  RobloxExtensionApp
    ...
    while (balanceString.find(token) != -1)
    {
        ...
        if(balanceString.find(token) != -1)
        {
			...
			for(unsigned i = 0; i < strlen(chars); ++i)
				key.erase(std::remove(key.begin(), key.end(), chars[i]), key.end());
            
            //val = 1818
            val = keyVal.substr(keyVal.find(":") + 1);

			for(unsigned i = 0; i < strlen(chars); ++i)
				val.erase(std::remove(val.begin(), val.end(), chars[i]), val.end());

			val.erase(std::remove_if(val.begin(), val.end(), (int(*)(int))isspace), val.end());
