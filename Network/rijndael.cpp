...
void InvMixColumn(word8 a[4][4], word8 BC) {
  // Mix the four bytes of every column in a linear way
    ...
    for(j = 0; j < BC; j++)
    {
        int a0 = a[j][0], a1 = a[j][1], a2 = a[j][2], a3 = a[j][3];
        a[j][0] = U1[a0][0] ^ U2[a1][0] ^ U3[a2][0] ^ U4[a3][0];
        a[j][1] = U1[a0][1] ^ U2[a1][1] ^ U3[a2][1] ^ U4[a3][1];
        a[j][2] = U1[a0][2] ^ U2[a1][2] ^ U3[a2][2] ^ U4[a3][2];
        a[j][3] = U1[a0][3] ^ U2[a1][3] ^ U3[a2][3] ^ U4[a3][3];
...
#ifdef _WIN32
#define RIJNDAEL_NOINLINE
#else
#define RIJNDAEL_NOINLINE __attribute__((noinline))
#endif

RIJNDAEL_NOINLINE void XOR4( unsigned char* dst, unsigned char* src0, unsigned char* src1 )
{
    dst[0] = src0[0] ^ src1[0];
    dst[1] = src0[1] ^ src1[1];
    dst[2] = src0[2] ^ src1[2];
...
RIJNDAEL_NOINLINE void XOR4_4( unsigned char* dst, unsigned char* src0, unsigned char* src1, unsigned char* src2, unsigned char* src3 )
{
    dst[0] = src0[0] ^ src1[0] ^ src2[0] ^ src3[0];
    dst[1] = src0[1] ^ src1[1] ^ src2[1] ^ src3[1];
    dst[2] = src0[2] ^ src1[2] ^ src2[2] ^ src3[2];
    dst[3] = src0[3] ^ src1[3] ^ src2[3] ^ src3[3];
}

RIJNDAEL_NOINLINE void MAGIC(word8 tk[MAXKC][4], int KC, int& rconpointer)
{
    int j = 0;
    tk[0][0] ^= S[tk[KC-1][1]];
    tk[0][1] ^= S[tk[KC-1][2]];
...
int rijndaelEncrypt (word8 a[16], word8 b[16], word8 rk[MAXROUNDS+1][4][4])
{
   ...
   word8 temp[4][4];
    ...
    XOR4( temp[0], (a+0), rk[0][0]);
    XOR4( temp[1], (a+4), rk[0][1]);
    XOR4( temp[2], (a+8), rk[0][2]);
    XOR4( temp[3], (a+12), rk[0][3]);
    
    XOR4_4( (b+0), T1[temp[0][0]], T2[temp[1][1]], T3[temp[2][2]], T4[temp[3][3]] );
    XOR4_4( (b+4), T1[temp[1][0]], T2[temp[2][1]], T3[temp[3][2]], T4[temp[0][3]] );
    XOR4_4( (b+8), T1[temp[2][0]], T2[temp[3][1]], T3[temp[0][2]], T4[temp[1][3]] );
    XOR4_4( (b+12), T1[temp[3][0]], T2[temp[0][1]], T3[temp[1][2]], T4[temp[2][3]] );
    
   for(r = 1; r < ROUNDS-1; r++) 
   {
       XOR4( temp[0], (b+0), rk[r][0]);
       XOR4( temp[1], (b+4), rk[r][1]);
       XOR4( temp[2], (b+8), rk[r][2]);
       XOR4( temp[3], (b+12), rk[r][3]);
   ...
   // last round is special   
    XOR4( temp[0], (b+0), rk[ROUNDS-1][0]);
    XOR4( temp[1], (b+4), rk[ROUNDS-1][1]);
    XOR4( temp[2], (b+8), rk[ROUNDS-1][2]);
    XOR4( temp[3], (b+12), rk[ROUNDS-1][3]);
   b[0] = T1[temp[0][0]][1];
   b[1] = T1[temp[1][1]][1];
   b[2] = T1[temp[2][2]][1]; 
   b[3] = T1[temp[3][3]][1];
   ...
   b[14] = T1[temp[1][2]][1]; 
   b[15] = T1[temp[2][3]][1];
    XOR4( (b+0), (b+0), rk[ROUNDS][0]);
    XOR4( (b+4), (b+4), rk[ROUNDS][1]);
    XOR4( (b+8), (b+8), rk[ROUNDS][2]);
    XOR4( (b+12), (b+12), rk[ROUNDS][3]);
