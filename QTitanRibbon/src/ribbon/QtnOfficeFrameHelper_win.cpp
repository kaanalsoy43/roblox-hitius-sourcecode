...
//        modifyStyle(m_hwndFrame, /*WS_BORDER|*/WS_CAPTION, WS_THICKFRAME, 0);
...
//                RECT rc;
//                ::GetWindowRect(m_hwndFrame, &rc);
                //::SetWindowPos(m_hwndFrame, Q_NULL, 0, 0, rc.right - rc.left, (rc.bottom - rc.top) + oldcustomMargins.top(), SWP_NOMOVE | SWP_NOZORDER | SWP_NOACTIVATE);
...
/*
        if ((lpwndpos->flags == (SWP_NOACTIVATE | SWP_NOZORDER)) && isTitleVisible())
        {
            ...
            else
            {
                if (isTheme2013())
                {
                    lpwndpos->x -= m_frameBorder/2;
                    lpwndpos->y -= m_frameBorder/2 - 3;
                    lpwndpos->cy -= m_frameBorder/2 + captionSize();
                }
                else
                {
                    lpwndpos->x -= m_frameBorder/2;//frameSize()/2;
                    lpwndpos->y -= m_frameBorder/2;//frameSize()/2;
                    lpwndpos->cy -= m_frameBorder/2 + captionSize();
                }
            }
        }
...
/*
#if (QT_VERSION >= QT_VERSION_CHECK(5, 0, 0))
        if (msg->message == WM_SIZE && (msg->wParam != m_changedSize))
        {
            collapseTopFrame();
            m_changedSize = msg->wParam;
        }
#endif // QT_VERSION >= QT_VERSION_CHECK(5, 0, 0)
*/
...
bool OfficeFrameHelperWin::eventFilter(QObject* obj, QEvent* event)
{
    if (obj == m_frame)
    {
        ...
        else if (event->type() == QEvent::/*UpdateRequest*/LayoutRequest)
        {
