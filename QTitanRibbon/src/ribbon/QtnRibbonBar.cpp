...
//      nLeft += 5;
    ...
    {
//      nLeft = m_rcQuickAccess.left() + (!m_quickAccessToolbarOnTop ? 2 : 0);
    }
...
QAction* RibbonBarPrivate::addTitleButton(QStyle::SubControl subControl, bool add, QRect& rcControl)
{
    ...
    else if (but && !add)
    {
        ...
        QCoreApplication::postEvent(but, new PostDeleteEvent());
//        delete but;
...
QMenu* RibbonBarPrivate::createContextMenu()
{
    ...
    {
        if (RibbonQuickAccessBar* quickAccessBar = p.getQuickAccessBar())
        {
            QList<QAction*> actions = quickAccessBar->actions();
            if (actions.size() > 0)
            {
                action = popup->addAction(RibbonBar::tr("Customize Quick Access Toolbar..."));
                action->setObjectName(strCustomizeQAToolBar);
                //                connect(action, SIGNAL(triggered()), &qtn_d(), SLOT(toggledQuickAccessToolbarOnTop()));

                action = popup->addAction(p.quickAccessBarPosition() == RibbonBar::QATopPosition ? RibbonBar::tr("Show Quick Access Toolbar Below the Ribbon") 
                    : RibbonBar::tr("Show Quick Access Toolbar Above the Ribbon"));
    ...
    {
        popup->addSeparator();
        action = popup->addAction(RibbonBar::tr("Minimize the Ribbon"));
...
*/
void RibbonBar::setLogoPixmap(Qt::AlignmentFlag alignment, QPixmap& logotype)
{
    QTN_D(RibbonBar);
...
int RibbonBar::availableWidth() const
{
	QTN_D(const RibbonBar);
	int leftSideWidth = 0;
	if (d.m_controlSystemButton)
		leftSideWidth = d.m_controlSystemButton->sizeHint().width();
	return parentWidget()->width() - (leftSideWidth + d.m_tabBar->tabBarWidth());
}
...
void RibbonBar::paintEvent(QPaintEvent* event)
{
    ...
    if (!d.m_minimized && rectEvent.intersects(opt.rect))
    {
...
void RibbonBar::setAllowPopups(bool allow)
{
    QTN_D(RibbonBar);

    d.m_allowPopups = allow;
}

bool RibbonBar::allowPopups() const
{
    QTN_D(const RibbonBar);

    return d.m_allowPopups;
}
