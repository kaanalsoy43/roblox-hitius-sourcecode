...
/*!
    \class Qtitan::PostDeleteEvent
    \internal
*/
static int postDeleteEventNumber = -1;
PostDeleteEvent::PostDeleteEvent() 
    : QEvent(PostDeleteEvent::eventNumber())
{
}

QEvent::Type PostDeleteEvent::eventNumber()
{
    if (postDeleteEventNumber < 0)
        postDeleteEventNumber = QEvent::registerEventType();
    return (QEvent::Type) postDeleteEventNumber;
}
...
QSize RibbonButton::sizeHint() const
{
    ...
    if (opt.toolButtonStyle != Qt::ToolButtonIconOnly)
    {
        if (opt.toolButtonStyle == Qt::ToolButtonTextUnderIcon)
        {
            ...
            h = style()->pixelMetric((QStyle::PixelMetric)RibbonStyle::PM_RibbonHeightGroup, 0, this) + 4;
			// END ROBLOX CHANGES
...
bool RibbonTitleButton::event(QEvent* event)
{
