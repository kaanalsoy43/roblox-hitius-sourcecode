...
//    ribbonDefaultGroupButton->deleteLater();
    m_reduced = reduced;
...
//            p.setWindowFlags(Qt::Popup);
        ...
        m_ribbonDefaultGroupButton->move(rect.topLeft());
    }
    else
    {
        if (m_ribbonDefaultGroupButton)
        {
            ...
            QWidget* pActivePopup = QApplication::activePopupWidget();
            if (pActivePopup && pActivePopup->isVisible())
                pActivePopup->hide();
            // END ROBLOX CHANGES
#endif
...
//            delete m_ribbonDefaultGroupButton;
            QCoreApplication::postEvent(m_ribbonDefaultGroupButton, new PostDeleteEvent());
...
/*
    ...
    switch (event->type()) 
    {
        case QEvent::FontChange: 
            if (!isReduced())
            {
                if (QWidget* parent = parentWidget()->parentWidget())
                {
                    QEvent ev(QEvent::LayoutRequest);
