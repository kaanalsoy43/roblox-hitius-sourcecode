...
void RibbonSystemPopupBar::paintEvent(QPaintEvent* event)
{
    ...
    if (count > 0)
    {
        ...
        StyleHintReturnThemeColor hintRectReturn(tr("Ribbon"), tr("MenuPopupRect"), QColor());
    ...
    for (int i = 0; i < count; i++)
    {
        if (RibbonPageSystemPopupListCaption* page = qobject_cast<RibbonPageSystemPopupListCaption*>(d.m_pageList.at(i)))
        {
            ...
            StyleHintReturnThemeColor hintColorReturn(tr("Ribbon"), tr("MenuPopupRect"), QColor());
...
void RibbonPageSystemRecentFileList::setSize(int size)
{
    ...
    if (size < 0)
    {
        Q_ASSERT_X(false, Q_FUNC_INFO, "Number of files cannot be less than 1.");
        return; 
    }
    // clean actions
