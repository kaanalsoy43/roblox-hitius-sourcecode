...
QSize RibbonTabPrivate::sizeForWidth(int w) const
{
    ...
    QSize contentsMargin(p.contentsMargins().left(), p.contentsMargins().right());
...
int RibbonTabBarPrivate::tabBarWidth() const
{
	QTN_P(const RibbonTabBar);
	...
	if (m_controlSystemButton)
	{
		totalWidth += m_controlSystemButton->sizeHint().width();
	}
	...
	if (QMenuBar* menuBar = qobject_cast<QMenuBar*>(p.parentWidget())) 
    {
        if (QWidget* rightCorner = menuBar->cornerWidget(Qt::TopRightCorner))
        {
            totalWidth += rightCorner->size().width();
		}
	}
	...
	{ 		
		RibbonTab* tab = m_tabList.at(i);
		if (tab->isVisible())
			totalWidth += tab->sizeHint().width() + 2*m_margin;
	}
