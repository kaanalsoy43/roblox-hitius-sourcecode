...
#else
namespace DFFlag {
	bool FixMatrixToAxisAngle = true;
}
#endif
...
void Matrix3::toAxisAngle (Vector3& rkAxis, float& rfRadians) const {
    ...
    if ((!DFFlag::FixMatrixToAxisAngle && rfRadians > 0.0f) || 
		(DFFlag::FixMatrixToAxisAngle && rfRadians > epsilon) ) {
        if ((!DFFlag::FixMatrixToAxisAngle  && rfRadians < pi()) || 
			(DFFlag::FixMatrixToAxisAngle && rfRadians < pi() - epsilon) ) {
			rkAxis.x = elt[2][1] - elt[1][2];
            rkAxis.y = elt[0][2] - elt[2][0];
            rkAxis.z = elt[1][0] - elt[0][1];
			if (DFFlag::FixMatrixToAxisAngle)
				rkAxis.unitize(0.0f);
			else
				rkAxis.unitize();
        } else {
...
// ROBLOX
Matrix3 Matrix3::fromAxisAngleFast(const Vector3& _normalizedAxis, float fRadians) {
...
//==============
    ...
    float fX2   = square(_normalizedAxis.x);
    float fY2   = square(_normalizedAxis.y);
    float fZ2   = square(_normalizedAxis.z);
    float fXYM  = _normalizedAxis.x * _normalizedAxis.y * fOneMinusCos;
    float fXZM  = _normalizedAxis.x * _normalizedAxis.z * fOneMinusCos;
    float fYZM  = _normalizedAxis.y * _normalizedAxis.z * fOneMinusCos;
    float fXSin = _normalizedAxis.x * fSin;
    float fYSin = _normalizedAxis.y * fSin;
    float fZSin = _normalizedAxis.z * fSin;
