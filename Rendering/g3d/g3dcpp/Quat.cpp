...
Quat Quat::fromRotation(const Vector3& rotation)
{
...
Quat Quat::fromVectors(const Vector3& from, const Vector3& to)
{
    Vector3 u = from.unit();
    Vector3 v = to.unit();
    Vector3 w = u.cross(v);
...
Quat::Quat(const Matrix3& rot) {
    if (DFFlag::G3DQuatConstructorFix)
    {
...
Vector3 Quat::operator*(const Vector3& other) const {
    Vector3 xyz(x, y, z);
    Vector3 t = 2 * xyz.cross(other);
    return other + w * t + xyz.cross(t);
}
